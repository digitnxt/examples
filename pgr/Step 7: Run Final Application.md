...
0. Hit access token
```bash
curl --location 'https://digit-lts.digit.org/keycloak/realms/SHILLONG/protocol/openid-connect/token' \
...
```
a. Hit the \_create API request to create a service request.
```bash
...
--data-raw '{
  "service": {
    ...
    "fileStoreId": "3b28b8ed-9b8c-4227-9166-810c545ff0bf",
...
b. Hit the \_search API request to search for the created service request.
```bash
...
c. Hit \_update API request to update your service request or transition it further in the workflow by changing the actions.
```bash
